...
from .models import StoreInventory, StoreStockMovement, StoreInventoryCount, StoreInventoryCountItem
...
class StoreStockMovementAdmin(admin.ModelAdmin):
    list_display = ['movement_id', 'store', 'product', 'movement_type', 'quantity', 'movement_date']
    list_filter = ['store', 'movement_type', 'movement_date']
    search_fields = ['movement_id', 'product__name', 'reference_id']
    readonly_fields = ['movement_id', 'total_cost', 'created_at']
